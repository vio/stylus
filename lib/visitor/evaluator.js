...
Evaluator.prototype.invokeFunction = function(fn, args, content){
  ...
  // function name
  scope.add(new nodes.Ident('function-name', fn.name));

  // called from
  scope.add(new nodes.Ident('called-from', this.getCalledFrom(this.stack)));
...
/**
 * Returns the function the current function was called from
 *
 * @param {Array} stack
 * @return {Node}
 * @api private
...
Evaluator.prototype.getCalledFrom = function(stack){
  var calledFrom = nodes.null;
  var counter = 0;
  for (var i = stack.length; i--;) {
    var frame = stack[i];
    if (frame._scope) {
      var functionName = frame._scope.locals['function-name'];
      if (functionName) {
        if (counter > 0) {
